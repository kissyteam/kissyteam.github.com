...
最常见的情况即是用 ``prop`` 来设置 ``disabled`` 以及 ``checked`` 而不是 :func:`attr` . 而 :func:`~DOM.val` 方法用来设置和读取 `value` property.    
...
.. note
    ...
    请使用 :func:`~DOM.data` 方法来处理自定义属性，而不要使用 ``prop`` 方法，否则在 ie<9 下会有内存泄露.    