...
## 为什么选择 KISSY
KISSY 诞生三年来历经淘宝的各种业务变化考验，证明了自己的可扩展性，高稳定性和可维护性，足以满足国内大多数的互联网业务。
...
KISSY 积极借鉴国际先进的框架类库设计，吸收 jquery 稳定的 dom 核心，yui 坚实的架构，extjs 优秀的组件设计，
并在移动设备上也有所支持，满足了淘宝从前台的 web page 到后台的 web app 再到移动端应用（起步阶段）的广泛需求。
同时 KISSY 也是一个完全自底向上开发起来的类库，存在时间还不长，在组件以及可测试性上尚有所欠缺，
...
ready（等待 dom ready）等常用的静态工具方法以及 Path，Uri，Promise 等模块化需要用到的基础类。
...
第三层为基础架构层。提供包括模拟 es5 属性管理的 base 模块，插件和多继承机制的 rich-base 模块以及具备一定逻辑的模板引擎 xtemplate。
...
包设计一方面通过约定优先配置的原则可以减少同一个包内多个模块的请求路径配置，
...
Overlay 是继承自 Component 然后静态得由一些分散的功能类扩展而来，包括定位功能类，对齐功能类，关闭功能类，遮罩层功能类，
这些功能类底层利用 javascript 的 mixin 功能将自己的方法和属性汇入到 Overlay 类中，使得最终的 Overlay 具备了这些能力。

组件在运行时也可以选择性得依赖某些插件模块，将插件的功能注入到组件中去，
...
随着用户越来越多得在移动设备上购物产生交易，移动应用在流量的比重上也越来越多，KISSY 及时适配移动设备，
目前在两方面进行了支持：
...
下一步 KISSY 会继续完善基础组件例如 date，datesource，selector，graphics 等，
重构已有组件例如 switchable，calendar 等，
增加 package manager 方便基于 KISSY 模块的提交与共享，补全测试用例，实现代码覆盖率检测，不断得提高运行稳定性。
