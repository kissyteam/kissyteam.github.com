...
> 注意：`//g.alicdn.com/`对应到`http://a.tbcdn.cn/g/`下的文件，比如，`//g.alicdn.com/abc.js`和`http://a.tbcdn.cn/g/abc.js`实际是一个文件。

### 1.4.x 将不再提供`kissy.js`

为了增强 KISSY 的模块架构和减轻体积，1.4.0 之后将不再提供粗粒度的`kissy.js`，因此`//g.alicdn.com/kissy/k/1.4.7/kissy.js`不存在。KISSY 将只提供`seed.js`。
